...
- Fixed inputs fetched from authenticated HTTP hosts (e.g. a private Artifactory mirror) failing with HTTP 401 when a Cachix auth token was also configured. devenv replaced the user's `netrc-file` with its Cachix-only netrc, and pointed Nix at it before the file was even written. The generated netrc now includes the user's netrc entries and exists before the first fetch.
